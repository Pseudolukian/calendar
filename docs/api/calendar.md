...
|----------|-----|----------|----------------|
| `tf_days` | `int` | Количество дней от текущей даты для начала фильтрации (0 — с завтрашнего дня, -1 — с сегодняшнего дня, 1 — с послезавтрашнего дня). | Нет. |
| `after_days` | `int` | Количество дней от текущей даты для окончания фильтрации (0 = до следующей недели, 1 = до завтра, 7 = до следующей недели и т.д.). | Нет. | 
...
=== "События за прошлую неделю до вчера"
...
=== "События за вчера"
    ...
    **Запрос**:
    ```curl
    curl -X 'GET' \
    'http://localhost:8000/api/Calendar?tf_days=-1&after_days=-1' \
    -H 'accept: application/json'
    ```
    ...
    [
        {
            ...
            "note": "Обсуждение требований к новой функциональности",
            "type": 1
        },
        {
            "id": 43,
            "start": "2025-07-24T14:00:00",
            "name": "Код-ревью PR #127",
            "note": "Проверка новых endpoint'ов календаря",
            "type": 2
        }
    ]
    ```
    ...
    **Ответ**:
    ...
    [
        ...
        },
        {
            ...
            "note": "Анализ выполненных задач и планов",
            "type": 1
        }
    ]
    ```
...
Если в параметр `tf_days` или `after_days` будет передан не `int` — вернётся ошибка:
